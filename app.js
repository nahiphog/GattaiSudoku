...
if (archiveHeading) { archiveHeading.textContent = "Daily archive"; archiveHeading.classList.remove("visually-hidden"); }
...
const solutionRail = document.createElement("aside"), solutionTitle = document.createElement("p"), stepControls = document.querySelector(".step-controls"), techniqueTallyButton = document.querySelector("#techniqueTally"); solutionRail.className = "solution-rail hidden"; solutionTitle.className = "solution-walkthrough-title"; solutionTitle.textContent = "Solution walkthrough"; guide.querySelector(".eyebrow")?.remove(); guide.before(solutionRail); solutionRail.append(solutionTitle, techniqueTallyButton, guide); guide.prepend(stepControls);
...
function renderStep() {
  ...
  // A shared cell has a coordinate in each grid.  The walkthrough already
  // identifies one grid, so retain only that grid's coordinate in its prose.
  const plainText = step.text
    .replace(/G1\s+(R\d+C\d+)\s*\/\s*G2\s+(R\d+C\d+)/g, (_match, one, two) => grid === 2 ? two : one)
    .replace(/In G[12],\s*/g, "")
    .replace(/\bG[12]\s+/g, "");
...
if (howToPlayDialog) {
  const difficultyGuideHtml = `<div class="table-wrap"><table class="difficulty-guide"><thead><tr><th>Difficulty</th><th>Techniques used</th><th>Scheduled days</th></tr></thead><tbody><tr><th scope="row">Easy · ≤800</th><td><span class="technique-name">Full House</span>, <span class="technique-name">Naked Single</span>, and <span class="technique-name">Hidden Single</span> only.</td><td>Monday · Tuesday</td></tr><tr><th scope="row">Medium · ≤1000</th><td>All earlier methods, plus <span class="technique-name">intersections</span> and <span class="technique-name">Naked or Hidden Pairs and Triples</span>.</td><td>Wednesday · Thursday · Friday</td></tr><tr><th scope="row">Tricky · ≤1150</th><td>All earlier methods. One Hard-class <span class="technique-name">fish, wing, or single-digit pattern</span> sets a Tricky floor.</td><td>Friday · Saturday</td></tr><tr><th scope="row">Hard · ≤1600</th><td>All earlier methods. Two or more Hard-class deductions set a Hard floor; cumulative score can raise it further.</td><td>Saturday</td></tr><tr><th scope="row">Unfair, Extreme, Nightmare · &gt;1600</th><td>All earlier methods. And <span class="technique-name">trial-and-error</span> is used.</td><td>Sunday</td></tr></tbody></table></div>`;
  const helpPages = [
...
(() => {
  ...
  window.syncGattaiLogoTheme = syncLogoTheme;
  const style = document.createElement("style");
