"""Generate a unique Gattai Sudoku whose dig never breaks a singles-only solve.

Singles are a strict subset of the user's permitted technique set:
Full House, Naked Single, Hidden Single, Locked Pair, Locked Triple,
Pointing and Claiming.  Therefore every accepted removal is solvable using
only allowed methods, with no search or contradiction used by the logic test.
"""
...
def count_solutions(givens,limit=2):
    ...
    def visit():
        ...
        for cell in active:
            ...
            cs=candidates(board,cell)
            if not cs:return 0
            if choices is None or len(cs)<len(choices):
                choice,choices=cell,cs
                if len(cs)==1:break
        if choice is None:return 1
        total=0
        for value in choices:
            board[choice]=value;total+=visit();board[choice]=0
    ...
    while True:
        ...
        if not move:
            for label,u in units:
                ...
                if move:break
        if not move and (naked_subset() or hidden_subset()):continue
        if not move:break
